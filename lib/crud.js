...
var crudTools=function(pDB)
{
    ...
    function insert(options, callback) {
        ...
        if (!options.data) {
            return callback(new ArgumentError("data"))
        }
        if (!options.collection) {
            return callback(new ArgumentError("collection"))
        }
        ...
        function collectionConnected(err, collection) {
            ...
            if (!collection) {
                return callback(new Error("Unable to load collection"))
            }
    ...
    function getById(options, callback) {
        ...
        if (!options.id) {
            return callback(new ArgumentError("id"))
        }
        getOptionDefaults(options);
    ...
    function aggregate(options, callback) {
        ...
        if (!options.pipeline) {
            return callback(new ArgumentError("pipeline"))
        }
        ...
        function collectionConnected(err, collection) {
            ...
            if (options.readPreference) {
                aggregateOptions.readPreference = options.readPreference;
            }
            try {
    ...
    function updatePart(options, callback) {
        ...
        function documentPatched(err, patchedDoc) {
            ...
            else if (patchedDoc == null || patchedDoc == 0) {
                return callback(null, 0)
            }
            else {
    ...
    function connectToCollection(options,callback){
        ...
        return callback(null,_db.collection(options.collection));
    }
...
function getIdQueryPart(options){
    ...
    var ret={};
    if (crudUtils.isValidId(idStr)){
        ret={_id:crudUtils.parseId(idStr)}
...
function prepQuery(query,options){
    ...
    if (options.rawQuery){
        var rawQuery=null;
        if (typeof options.rawQuery==="string"){
            rawQuery=JSON.parse(options.rawQuery);
        }
        else{
            rawQuery=options.rawQuery;
        }

        for (var k in rawQuery ) {
