...
var crudTools=function(connectionString,connectionOptions,mainCallback)
{
    if (!mainCallback){mainCallback=function(){}}

    var _db=null;

    mongo.MongoClient.connect(connectionString, connectionOptions, function(err, database) {
        if(err){return mainCallback(err);}
        _db = database;
        _db.on('close', function() {
            console.error("connection forced closed ");
        });
        _db.on('error', function(err) {
            var strErr=err?(err.message?err.message:err):"";
            console.error("connection error: " +  JSON.stringify(strErr));

        });
        _db.on('timeout', function(err) {
            var strErr=err?(err.message?err.message:err):"";
            console.error("connection timed out:"  + JSON.stringify(strErr));
        });
...
module.exports=function(connectionString,connectionOptions,callback){
    return new crudTools(connectionString,connectionOptions,callback);
}
